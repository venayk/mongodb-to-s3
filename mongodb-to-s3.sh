...
backup() {
  ...
  mongodump --host $DATABASE_HOST:$DATABASE_PORT --db $DATABASE --out $BACKUP_PATH
  if [ $? -ne 0 ]; then
...
restore() {
 ...
 # if influxd restore -database $DATABASE -datadir $DATABASE_DATA_DIR -metadir $DATABASE_META_DIR $BACKUP_PATH ; then
  if mongorestore --host $DATABASE_HOST:$DATABASE_PORT --db $DATABASE  "${BACKUP_PATH}"/${DATABASE}; then
